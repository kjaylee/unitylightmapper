...
public class LightmappingTool : EditorWindow
{
    ...
    //if you don't like the logo taking your precious screen space, then don't throw this one
    ...
    private void ChangeIncompatible()
    {
        if (allData[currentLightmap] != null && ((ArrayList)allData[currentLightmap]).Count > 0)
        {
            ...
            Shader sh = Shader.Find("ExternalLightmappingTool/LightmappedDiffuse");
            if (sh == null)
            {
                UnityEngine.Debug.LogWarning("ExternalLightmappingTool/LightmappedDiffuse shader not found!");
        ...
        if (bigArray.Count > 0)
        {
            ...
            if (Done)
            {
                ...
                if ((appPaths.Count > 0) && !turnedToMax)
                {
                    ...
                    {
                        string presetFile="";
                        if (File.Exists("" +MaxFiles + Path.GetFileNameWithoutExtension(EditorApplication.currentScene) + ".max"))
                        {
                            presetFile = "\"" + MaxFiles + Path.GetFileNameWithoutExtension(EditorApplication.currentScene) + ".max\"";
                        }
                        string MaxParameters = " " + presetFile + " -U MAXScript \"" + MaxFiles + Path.GetFileNameWithoutExtension(EditorApplication.currentScene) + ".ms\"";
